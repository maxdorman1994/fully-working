...
DROP TABLE IF EXISTS journal_likes CASCADE;
DROP TABLE IF EXISTS journal_comments CASCADE;
DROP VIEW IF EXISTS journal_entry_stats CASCADE;

-- First, let's check what type the journal_entries.id actually is
-- and create our tables to match

-- Create comments table - using TEXT for journal_entry_id to handle both UUID and integer
...
-- Create likes table - using TEXT for journal_entry_id to handle both UUID and integer  
...
-- Using TEXT for journal_entry_id to match our tables
...
) c ON je.id::TEXT = c.journal_entry_id
LEFT JOIN (
    SELECT journal_entry_id, COUNT(*) as like_count
    FROM journal_likes
    GROUP BY journal_entry_id
) l ON je.id::TEXT = l.journal_entry_id;
